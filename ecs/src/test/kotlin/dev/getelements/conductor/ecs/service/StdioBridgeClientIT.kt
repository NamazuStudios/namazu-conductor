...
class StdioBridgeClientIT {
    ...
    // The self-restarting bridge container in CI (see release.yaml/ecs-it.yaml) is a tight
    // `docker run --rm` loop with no guaranteed restart latency. In release.yaml specifically, the
    // reactor also builds edgegap (whose own StdioBridgeClientIT kills the same shared container
    // right before this one needs it) while minikube runs concurrently — under that combined load
    // the restart can occasionally lag past its usual near-instant respawn. Tolerate a brief window
    // here rather than fail on a container that is merely still coming back up.
